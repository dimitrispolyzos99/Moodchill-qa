...
Automated Postman tests for the 4 REST APIs used in MoodChill, an iOS app I built. The suite covers positive and negative cases: status codes, response structure, and authentication validation.
...
How to run
Import MoodChill-API-Tests.postman_collection.json into Postman.
For the Quotes requests, add your own API-Ninjas key in the X-Api-Key header. The key is not included in this repo for security — the placeholder YOUR_API_KEY_HERE marks where it goes.
Send each request, or use the Collection Runner to run the whole suite at once and view the test results.
